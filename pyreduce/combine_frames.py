...
def combine_frames(files, instrument, mode, extension=None, dtype=np.float32,combine_type='mean', **kwargs):
    """
    ...
    if combine_type=='mean':
        ...
        raise ValueError(
            "Unexpected value for 'combine_types', expected one of ['mean', 'median'], but got %s"% combine_type
        )
...
#         positive probabilities we compute the ratios of the original data to the probabilities and get
...
def combine_calibrate(
    ...
    **kwargs,
    ...
    orig, thead = combine_frames(files, instrument, mode, mask=mask,combine_type=combine_type, **kwargs)
...
def combine_bias(
    ...
    **kwargs,
    ...
    bias1, head1 = combine_frames(list1, instrument, mode, extension,combine_type='median', **kwargs)
    bias1 /= n1

    bias2, head = combine_frames(list2, instrument, mode, extension,combine_type='median', **kwargs)
    bias2 /= n2
    ...
    bias,_ = combine_frames(files, instrument, mode, extension, combine_type='median', **kwargs)
    bias = bias/n
